...
if CG:FindFirstChild("TeleMenu") then CG.TeleMenu:Destroy() end
local G = Instance.new("ScreenGui", CG); G.Name = "TeleMenu"; G.ResetOnSpawn = false
...
spawn(function() while task.wait() do F.BorderColor3 = Color3.fromHSV(tick() % 5 / 5, 1, 1) end end)
local Title = Instance.new("TextLabel", F)
Title.Text = "TELEPORT MENU"; Title.Size = UDim2.new(1, 0, 0, 40); Title.BackgroundTransparency = 1
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP then
            ...
            b.MouseButton1Click:Connect(function()
                pcall(function() LP.Character.HumanoidRootPart.CFrame = p.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3) end)
            end)
        end
    end
...
Tgl.MouseButton1Click:Connect(function() F.Visible = not F.Visible; Tgl.Text = F.Visible and "CLOSE" or "OPEN" end)
